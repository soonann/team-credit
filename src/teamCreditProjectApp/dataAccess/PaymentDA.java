...
public class PaymentDA {
	...
	private static Payment convertToPayment(ResultSet rs) throws SQLException {
		Payment payment;
		int id = rs.getInt("id");
		...
		double actualPayment = rs.getDouble("actualPayment");
		payment = new Payment(id,cardNumber,startDate,endDate,minPayment,actualPayment);
	...
	*/
	...
	public static ArrayList<Payment> retrieveAllPayment() {
		// declare local variables
		...
		try {
			while (rs.next()) {
				Payment payment = convertToPayment(rs);
				list.add(payment);
			}
		} catch (Exception e) {
			e.printStackTrace();
